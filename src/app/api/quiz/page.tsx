...
export default function QuizPage() {
  ...
  async function claimQuizPoints(finalResults: boolean[]) {
    if (!userId) return;
    try {
      const res = await fetch("/api/quiz/complete", {
        ...
        body: JSON.stringify({
          correctCount: finalResults.filter(Boolean).length,
      ...
      /* abaikan kegagalan klaim poin agar kuis tetap selesai */
    }
  }
    ...
    <main className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="mx-auto max-w-2xl px-6 pt-6">
        {loading ? (
          ...
          <Summary correctCount={correctCount} total={total} pointsEarned={pointsEarned} />
        ) : (
...
function Summary({
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-8 text-center shadow-sm">
      ...
      {pointsEarned !== null && (
        <div
          className="mx-auto mt-3 inline-block rounded-full px-5 py-2 text-base font-bold text-gray-900"
          style={{ backgroundColor: YELLOW }}
        >
          +{pointsEarned} poin! 🎉
        </div>
